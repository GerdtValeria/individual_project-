...
class SRent(BaseModel):
    ...
    id_image: int = Field(..., ge=1)
    id_user: int = Field(..., ge=1)
    title: str = Field(...)
    price: int = Field(..., min_length=1, max_length=50, description="Имя студента, от 1 до 50 символов")
    description: str = Field(..., min_length=1, max_length=50, description="Фамилия студента, от 1 до 50 символов")
    active: date = Field(..., description="Дата рождения студента в формате ГГГГ-ММ-ДД")
